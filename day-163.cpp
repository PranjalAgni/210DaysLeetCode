...
class Solution {
   public:
    string getHint(string secret, string guess) {
        // magic lines to beat runtime
        ios_base::sync_with_stdio(false);
        cin.tie(NULL);

        int digitsMatchAtSamePosition = 0;
        int digitMatchAtWrongPosition = 0;
        unordered_map<char, int> missedWordsSecret;
        unordered_map<char, int> missedWordsGuess;

        for (int idx = 0; idx < secret.size(); idx++) {
            if (guess[idx] == secret[idx])
                digitsMatchAtSamePosition += 1;
            else {
                bool isSecretFound = false;
                bool isGuessFound = false;
                if (missedWordsSecret[guess[idx]] > 0) {
                    digitMatchAtWrongPosition += 1;
                    missedWordsSecret[guess[idx]] -= 1;
                    isGuessFound = true;
                }

                if (missedWordsGuess[secret[idx]] > 0) {
                    digitMatchAtWrongPosition += 1;
                    missedWordsGuess[secret[idx]] -= 1;
                    isSecretFound = true;
                }

                if (!isSecretFound) missedWordsSecret[secret[idx]] += 1;
                if (!isGuessFound) missedWordsGuess[guess[idx]] += 1;
            }
        }

        return to_string(digitsMatchAtSamePosition) + 'A' +
               to_string(digitMatchAtWrongPosition) + 'B';
    }
};
