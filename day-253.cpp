...
// Beats 99.81 % of cpp submissions.
...
class BSTIterator {
   public:
    ...
    void bstHelper(TreeNode* root, vector<int>& inorderVals) {
        if (root == NULL) return;

        bstHelper(root->left, inorderVals);
        inorderVals.push_back(root->val);
        bstHelper(root->right, inorderVals);
    }

    BSTIterator(TreeNode* root) {
        inorderVals = vector<int>();
        cursor = 0;
        bstHelper(root, inorderVals);
        size = inorderVals.size();
    }

    int next() {
        cursor += 1;
        return inorderVals[cursor - 1];
    }

    bool hasNext() { return cursor < size; }
};
