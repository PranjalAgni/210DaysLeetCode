...
// Simple O(N) solution
// Beats 98.39 % of cpp submissions.
...
class Solution {
   public:
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        ...
        int endDurationTime = timeSeries[0] + duration - 1;

        for (int idx = 1; idx < N; idx++) {
            int nextEndDuraton = timeSeries[idx] + duration - 1;
            if (timeSeries[idx] > endDurationTime) {
                totalDuration += duration;
                endDurationTime = nextEndDuraton;
            } else if (nextEndDuraton >= endDurationTime) {
                totalDuration += (nextEndDuraton - endDurationTime);
                endDurationTime = nextEndDuraton;
            }
        }
