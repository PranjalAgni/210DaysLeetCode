...
cell becomes an empty cell (0). When both robots stay on the same cell, only one
of them takes the cherries. Both robots cannot move outside of the grid at any
moment. Both robots should reach the bottom row in the grid.
...
by Robot #2, (1 + 3 + 4 + 3) = 11. Total of cherries: 17 + 11 = 28. Example 3:
...
class Solution {
   ...
   public:
    ...
    int getMaxCherries(vector<vector<int>>& grid,
                       vector<vector<vector<int>>>& memo, int i, int j1,
                       int j2) {
        if (i >= grid.size()) {
            return 0;
        }
        ...
        for (auto dj1 : dirs) {
            ...
            for (auto dj2 : dirs) {
                ...
                count = max(count, getMaxCherries(grid, memo, i + 1, j1 + dj1,
                                                  j2 + dj2));
            }
        }

        memo[i][j1][j2] =
            count + (j1 == j2 ? grid[i][j1] : grid[i][j1] + grid[i][j2]);
